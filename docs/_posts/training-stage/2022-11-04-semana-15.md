...
Una vez localizados los sensores y actuadores hemos de ser capaces de modificarlos y provocar cambios en la escena. ¿Cómo podemos visualizarlo? Búsqueda de *print()* o *read()*, aunque también existen trucos como modificar los movimientos del robot en función de nuestras alteraciones.

- robots\robotAPI\consoleRobotAPI.js 

En esta ruta podemos encontrar dos funciones. Por un lado, tenemos *printConsole(texto, myRobot)* y, por otro lado, tenemos *inputConsole(texto, myRobot)*.

¿Cúando son utilizadas estas funciones?

Ambas son importadas en interfacesRobot.js, pero parecen no ser utilizadas como tal. 