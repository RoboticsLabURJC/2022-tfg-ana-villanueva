...
> No básicas (hacen uso de las anteriores)
...
Existen otras funciones, pero no se para que sirven. Por otro lado, las funciones básicas parecen funcionar, ya que he obtenido sus valores mediante getV, getW y getL. ¿Por qué no funciona el movimiento?
El único hilo que veo con las funciones básicas es que el mini-proxy es quien envía el valor a la función setV, por ejemplo. (Esto se realiza correctamente y no parece ser el problema).
